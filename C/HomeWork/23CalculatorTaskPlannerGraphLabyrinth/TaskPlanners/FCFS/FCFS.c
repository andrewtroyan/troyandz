...
{
    pointer->node = NULL;
    pointer->amountOfFinishedProcesses = 0;
}

bool addToListOfFinishedProcesses(FinishedProcess data, ListOfFinishedProcesses *pointer)
{
    FinishedNode *newData = NULL, *tempPointer = pointer->node;

    newData = (FinishedNode *)malloc(sizeof(FinishedNode));
    if(newData == NULL)
        return false;
    ...
    ++(pointer->amountOfFinishedProcesses);
    newData = NULL;
    return true;
}
...
{
    FinishedNode *tempPointer = pointer->node;
    pointer->amountOfFinishedProcesses = 0;
    while(pointer->node)
    {
        tempPointer = pointer->node->link;
        free(pointer->node);
        pointer->node = tempPointer;
    }
}

int amountOfFinishedProcesses(ListOfFinishedProcesses pointer)
{
    return pointer.amountOfFinishedProcesses;
}
